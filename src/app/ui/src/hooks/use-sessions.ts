    ...
    try {
      ...
      if (enableOptimizations) {
        ...
        if (cached && cached.isLoaded) {
          console.log('✅ Using cached session data for', sessionId, 'with', cached.messages?.length || 0, 'messages')
          // Use cached data
        ...
        queryClient.setQueryData(sessionQueryKeys.lists(), (oldSessions: Session[] = []) => {
          return oldSessions.map(session => 
            session.id === sessionId 
              ? { ...session, lastActivity: new Date().toISOString(), messageCount: (session.messageCount || 0) + 1 }
              : session
          )
        })
