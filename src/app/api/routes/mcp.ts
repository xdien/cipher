...
export function createMcpRoutes(agent: MemAgent): Router {
	...
	router.delete('/servers/:serverId', validateMcpServerId, async (req: Request, res: Response) => {
		try {
			...
			if (!serverId) {
				errorResponse(res, ERROR_CODES.BAD_REQUEST, 'Server ID is required', 400, undefined, req.requestId);
				return;
			}
	 ...
	 */
	router.get('/servers/:serverId/tools', validateMcpServerId, async (req: Request, res: Response) => {
		try {
			const { serverId } = req.params;
	 ...
	 */
	router.post('/servers/:serverId/tools/:toolName/execute', validateToolExecution, async (req: Request, res: Response) => {
		try {
			const { serverId, toolName } = req.params;
			
			if (!serverId || !toolName) {
				errorResponse(res, ERROR_CODES.BAD_REQUEST, 'Server ID and tool name are required', 400, undefined, req.requestId);
				return;
			}
