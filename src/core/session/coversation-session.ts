...
export class ConversationSession {
  ...
  constructor(
    ...
    if (options?.enableHistory !== false && services.storageManager && services.logger) {
      this.historyProvider = createDatabaseHistoryProvider(services.storageManager, services.logger);
    }
  }
  ...
  public async run(
    ...
    if (options && Object.keys(options).some(k => !['memoryMetadata', 'contextOverrides'].includes(k))) {
      // eslint-disable-next-line no-console
  ...
  public getLLMService() { return this.llmService; }
  public getUnifiedToolManager() { return this.services.unifiedToolManager; }
}
