...
import { ProviderRegistry, ProviderConfig, PromptProvider, ProviderGenerator, ProviderType } from './interfaces.js';
...
export class DefaultProviderRegistry implements ProviderRegistry {
  ...
  public async create(config: ProviderConfig): Promise<PromptProvider> {
    ...
    try {
      ...
      throw new Error(`Failed to create provider '${config.name}': ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
