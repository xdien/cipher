...
export type DynamicContentGenerator = (context: ProviderContext, config: Record<string, any>) => Promise<string>;
...
export class DynamicPromptProvider extends BasePromptProvider {
  ...
  public async generateContent(context: ProviderContext): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Failed to generate dynamic content: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
