...
export class FilePromptProvider extends BasePromptProvider {
  ...
  private async loadFileContent(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to load file ${this.filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async checkAndReloadFile(): Promise<void> {
    try {
      ...
      console.warn(`Failed to check file ${this.filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}