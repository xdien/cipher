 ...
 */
export const timestampGenerator: DynamicContentGenerator = async (context: ProviderContext, config: Record<string, any>): Promise<string> => {
 ...
 */
export const sessionContextGenerator: DynamicContentGenerator = async (context: ProviderContext, config: Record<string, any>) => {
  ...
  switch (format) {
    ...
    case 'json':
      ...
      if (includeFields.includes('sessionId') && context.sessionId) obj.sessionId = context.sessionId;
 ...
 */
export const memoryContextGenerator: DynamicContentGenerator = async (context: ProviderContext, config: Record<string, any>) => {
 ...
 */
export const environmentGenerator: DynamicContentGenerator = async (context: ProviderContext, config: Record<string, any>) => {
  ...
  return customMessages[environment] || defaultMessages[environment as keyof typeof defaultMessages] || `Environment: ${environment}`;
};

/**
 * Generate conditional content based on context
 */
export const conditionalGenerator: DynamicContentGenerator = async (context: ProviderContext, config: Record<string, any>) => {
