...
export class SystemPromptConfigManager {
  ...
  public async loadFromFile(filePath: string, options: ConfigLoadOptions = {}): Promise<void> {
    const fullPath = path.isAbsolute(filePath) ? filePath : path.resolve(options.baseDir || process.cwd(), filePath);
  ...
  private validateProvider(provider: any, index: number): void {
    ...
    if (typeof provider.type !== 'string' || !Object.values(ProviderType).includes(provider.type as ProviderType)) {
      throw new Error(`${prefix} must have a valid "type" (${Object.values(ProviderType).join(', ')})`);
    }
    ...
    if (provider.config !== undefined && (typeof provider.config !== 'object' || provider.config === null)) {
   ...
   */
  private processConfiguration(config: SystemPromptConfig, options: ConfigLoadOptions): SystemPromptConfig {
