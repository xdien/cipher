...
export class HybridStrategy implements ICompressionStrategy {
    ...
    async compress(
        ...
        const conversationAnalysis = this.analyzeConversation(processedMessages, currentTokenCount, targetTokenCount);
        ...
        if (conversationAnalysis.recommendedStrategy === 'middle-removal') {
            ...
            result = await this.middleRemovalStrategy.compress(messages, currentTokenCount, targetTokenCount);
        } else if (conversationAnalysis.recommendedStrategy === 'oldest-removal') {
            logger.debug('Hybrid strategy choosing oldest-removal');
            result = await this.oldestRemovalStrategy.compress(messages, currentTokenCount, targetTokenCount);
        } else {
    ...
    private async adaptiveCompress(
        ...
        const middleResult = await this.middleRemovalStrategy.compress(messages, currentTokenCount, targetTokenCount);
        const oldestResult = await this.oldestRemovalStrategy.compress(messages, currentTokenCount, targetTokenCount);
