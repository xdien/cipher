...
export class MiddleRemovalStrategy implements ICompressionStrategy {
    ...
    async compress(
        ...
        const endPreserveCount = Math.min(this.config.preserveEnd, sortedRemovable.length - startPreserveCount);
        
        const startMessages = sortedRemovable.slice(0, startPreserveCount);
        const endMessages = sortedRemovable.slice(-endPreserveCount);
        const middleMessages = sortedRemovable.slice(startPreserveCount, sortedRemovable.length - endPreserveCount);
