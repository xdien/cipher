...
export class AzureService implements ILLMService {
  ...
  constructor(
    ...
    if (!endpoint) {
      throw new Error('Azure OpenAI endpoint is required. Provide via config.azure.endpoint or AZURE_OPENAI_ENDPOINT environment variable');
    }
  ...
  private async getAIResponse(formattedTools: any[]): Promise<any> {
    ...
    if (formattedTools.length > 0) {
      ...
      const validFunctions = formattedTools.filter(func => 
        func && func.name && func.description && func.parameters
      );
