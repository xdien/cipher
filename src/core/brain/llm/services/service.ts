...
export class LLMServices implements ILLMService {
    ...
    constructor(
        ...
        if (config.provider === 'aws') {
            ...
            const arn = config.inferenceProfileArn || config.awsConfig?.inferenceProfileArn || process.env.AWS_BEDROCK_INFERENCE_PROFILE_ARN;
            if (arn !== undefined) {
    ...
    private initializeClient(config: ExtendedLLMConfig): any {
        switch (config.provider) {
            ...
            case 'aws':
                ...
                const accessKeyId = config.awsConfig?.accessKeyId || 
                                  process.env.AWS_ACCESS_KEY_ID;
                const secretAccessKey = config.awsConfig?.secretAccessKey || 
                                      process.env.AWS_SECRET_ACCESS_KEY;
                const sessionToken = config.awsConfig?.sessionToken || 
    ...
    private async generateOpenAICompatible(userInput: string, imageData?: ImageData, stream?: boolean): Promise<string> {
        const formattedTools = await this.getFormattedTools('openai');
        logger.silly(`Formatted tools for ${this.config.provider}: ${JSON.stringify(formattedTools, null, 2)}`);
        ...
        while (iterationCount < this.maxIterations) {
            ...
            const { message } = await this.getOpenAIResponseWithRetries(formattedTools, userInput, stream);
    ...
    private async generateAnthropic(userInput: string, imageData?: ImageData, stream?: boolean): Promise<string> {
        ...
        while (iterationCount < this.maxIterations) {
            ...
            const { response } = await this.getAnthropicResponseWithRetries(formattedTools, userInput, stream);
    ...
    private async generateGoogle(userInput: string, imageData?: ImageData): Promise<string> {
    ...
    private async generateBedrock(userInput: string, imageData?: ImageData): Promise<string> {
    ...
    private async generateAzure(userInput: string, imageData?: ImageData, stream?: boolean): Promise<string> {
    ...
    private async generateGroq(userInput: string, imageData?: ImageData, stream?: boolean): Promise<string> {
    ...
    private async getOpenAIResponseWithRetries(tools: any[], userInput: string, stream?: boolean): Promise<{ message: any }> {
        let attempts = 0;
    ...
    private async getAnthropicResponseWithRetries(tools: any[], _userInput: string, stream?: boolean): Promise<{ response: any }> {
        let attempts = 0;
        const MAX_ATTEMPTS = 3;
        ...
        for (const toolCall of message.tool_calls) {
            try {
                ...
                if(this.unifiedToolManager){
                    ...
                    toolResult = await this.mcpManager.executeTool(toolCall.function.name, JSON.parse(toolCall.function.arguments));
                }
                ...
                if (typeof this.contextManager.addToolResult === 'function') {
                    ...
                    logger.warn('contextManager.addToolResult is not available, skipping tool result storage');
                }
            } catch (error) {
                const errorMessage = error instanceof Error? error.message: String(error);
...
private async getFormattedTools(provider: 'openai' | 'anthropic' | 'openrouter' | 'aws' | 'azure' | 'qwen' | 'gemini' | 'groq'): Promise<any[]> {
    try {
