...
describe('AzureService', () => {
  ...
  describe('generate', () => {
    it('should call addUserMessage and return string', async () => {
      mockOpenAIClient.getChatCompletions.mockResolvedValue({ choices: [{ message: { content: 'response' } }] });
      // Patch getAIResponse to return a compatible response
      (azureService as any).getAIResponse = vi.fn().mockResolvedValue({ choices: [{ message: { content: 'response' } }] });
