...
describe('AwsService', () => {
  ...
  describe('generate', () => {
    it('should call addUserMessage and return string', async () => {
      mockBedrockClient.send.mockResolvedValue({ body: Buffer.from(JSON.stringify({ content: [{ type: 'text', text: 'response' }] })) });
  ...
  describe('directGenerate', () => {
    it('should return string response', async () => {
      mockBedrockClient.send.mockResolvedValue({ body: Buffer.from(JSON.stringify({ content: [{ type: 'text', text: 'direct response' }] })) });
