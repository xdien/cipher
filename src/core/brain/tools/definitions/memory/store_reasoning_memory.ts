...
export const storeReasoningMemoryTool: InternalTool = {
  ...
  parameters: {
    ...
    properties: {
      trace: {
        type: 'object',
        description: 'Complete reasoning trace from cipher_extract_reasoning_steps (includes steps and metadata with task context)',
        properties: {
          id: { type: 'string' },
          steps: {
            type: 'array',
            ...
            properties: {
              ...
              taskContext: {
                ...
                properties: {
                  ...
                  input: { type: 'string', description: 'Original user request or problem statement' },
                  taskType: { type: 'string', description: 'Type of task (e.g., code_generation, analysis, problem_solving)' },
                  domain: { type: 'string', description: 'Problem domain (e.g., programming, math, planning)' },
  ...
  handler: async (args: any, context?: InternalToolContext) => {
    ...
    logger.debug('StoreReasoningMemory: Processing unified reasoning storage request', {
      ...
      qualityScore: args.evaluation?.qualityScore || args.evaluation?.result?.evaluation?.qualityScore,
      hasTrace: !!args.trace,
      hasEvaluation: !!args.evaluation,
      hasTaskContext: !!(args.trace?.metadata?.taskContext || args.trace?.result?.trace?.metadata?.taskContext)
    });
    ...
    try {
      ...
      if (!trace) {
        validationErrors.push('trace parameter is missing - ensure cipher_extract_reasoning_steps was called successfully');
      ...
      if (!evaluation) {
        validationErrors.push('evaluation parameter is missing - ensure cipher_evaluate_reasoning was called successfully');
      ...
      if (validationErrors.length > 0) {
        ...
        logger.warn('StoreReasoningMemory: Validation failed, suggesting pipeline check', {
          ...
          pipelineStage: !trace ? 'extraction_failed' : !evaluation ? 'evaluation_failed' : 'unknown'
        });
        ...
        return {
          ...
          result: { 
            ...
            stored: false,
            validationErrors,
            suggestion: 'Ensure both cipher_extract_reasoning_steps and cipher_evaluate_reasoning completed successfully before calling store_reasoning_memory'
          },
          metadata: { 
      ...
      const validSteps = trace.steps.filter((step: any) => 
        step && typeof step === 'object' && step.content && step.type
      );
      ...
      if (validSteps.length === 0) {
        ...
        return {
          success: false,
          result: { 
            error: 'No valid reasoning steps found - all steps are missing required fields (type, content)',
            stored: false 
          },
          metadata: { 
            toolName: 'store_reasoning_memory',
      ...
      if (evaluation.shouldStore === false) {
        logger.debug('StoreReasoningMemory: Skipping storage - evaluation indicates should not store');
        return {
          success: true,
          result: { 
      ...
      const safeQualityScore = typeof evaluation.qualityScore === 'number' 
      ...
      const invalidEmbeddingElements = embedding.filter((val, idx) => 
      ...
      try {
        ...
        // Method 2: Direct Qdrant client access  
          ...
          const collection = await (reflectionStore as any).client.getCollection(reflectionCollectionName);
          collectionExists = true;
        ...
        logger.error('StoreReasoningMemory: Collection verification failed - attempting auto-creation', {
          ...
          errorType: collectionError instanceof Error ? collectionError.constructor.name : typeof collectionError
        });
        
        // EMERGENCY: Try to create/recreate the collection
        try {
          console.error('=== COLLECTION AUTO-CREATION ATTEMPT ===');
          console.error('Collection Error:', collectionError instanceof Error ? collectionError.message : String(collectionError));
          ...
          console.error('Creation Error:', creationError instanceof Error ? creationError.message : String(creationError));
          ...
          throw new Error(`Reflection collection '${env.REFLECTION_VECTOR_STORE_COLLECTION}' does not exist and cannot be created: ${creationError instanceof Error ? creationError.message : String(creationError)}`);
        }
      }
      ...
      const storeInfo = {
        ...
        isConnected: typeof reflectionStore.isConnected === 'function' ? reflectionStore.isConnected() : 'unknown',
        collectionExists,
        collectionCreated,
        collectionName: env.REFLECTION_VECTOR_STORE_COLLECTION,
      ...
      try {
        ...
        logger.debug('StoreReasoningMemory: ADD operation completed', {
          memoryId: vectorId,
          textPreview: searchableContent.substring(0, 60) + (searchableContent.length > 60 ? '...' : ''),
          stepCount: validSteps.length,
        ...
        console.error('VectorId:', vectorId, 'Type:', typeof vectorId, 'IsInteger:', Number.isInteger(vectorId));
        console.error('Embedding Length:', Array.isArray(embedding) ? embedding.length : 'NOT_ARRAY');
        ...
        console.error('Actual Embedding Dimension:', Array.isArray(embedding) ? embedding.length : 'NOT_ARRAY');
        console.error('Dimension Match:', Array.isArray(embedding) && embedding.length === env.VECTOR_STORE_DIMENSION);
        console.error('Insert Error:', insertError instanceof Error ? insertError.message : String(insertError));
        
        // EXTRACT UNDERLYING QDRANT ERROR
        const underlyingError = (insertError as any)?.cause || (insertError as any)?.originalError;
        if (underlyingError) {
          console.error('Underlying Qdrant Error:', underlyingError instanceof Error ? underlyingError.message : String(underlyingError));
          console.error('Qdrant Error Stack:', underlyingError instanceof Error ? underlyingError.stack : 'No stack available');
        }
        ...
        const detailedError = insertError instanceof Error ? insertError.message : String(insertError);
        ...
        logger.error('StoreReasoningMemory: Vector store insert failed with comprehensive details', {
          traceId: trace.id,
          vectorId,
          ...
          embeddingLength: Array.isArray(embedding) ? embedding.length : 'not-array',
          embeddingHasValidNumbers: Array.isArray(embedding) && embedding.every(x => typeof x === 'number' && isFinite(x)),
