...
export const WebSearchConfigSchema = z.discriminatedUnion('engine', [
    DuckDuckGoSchema,
]);
...
const WebSearchInputSchema = z.object({
    ...
    max_results: z.number().min(1).max(20).default(10).optional().describe('Maximum number of search results to return (1-20)'),
    safe_mode: z.boolean().default(true).optional().describe('Enable safe search mode to filter explicit content'),
});
