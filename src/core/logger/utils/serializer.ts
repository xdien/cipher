...
export function truncateForLogging(
  ...
  if (Array.isArray(obj)) {
    const truncated = obj.slice(0, maxArrayLength).map(item => 
      truncateForLogging(item, maxStringLength, maxArrayLength, maxObjectKeys)
    );
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    for (const key of keysToProcess) {
      truncated[key] = truncateForLogging(
        obj[key], 
        maxStringLength, 
        maxArrayLength, 
        maxObjectKeys
