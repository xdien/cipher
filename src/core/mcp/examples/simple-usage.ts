...
export async function oneOffOperationExample() {
  ...
  try {
    ...
    const result = await withTemporaryClient(
      ...
      serverConfig,
      async (client) => {
        console.log('Connected to test server for one-off operation');
    ...
    console.error('One-off operation failed (expected for demo server):', error);
  }
}
...
export async function multiServerAggregationExample() {
  ...
  try {
    ...
    console.log(`Aggregator stats: ${stats.serverCount} servers, ${stats.totalOperations} operations`);
