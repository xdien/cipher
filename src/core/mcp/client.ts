...
export class MCPClient implements IMCPClient {
  ...
  private async _performConnection(config: McpServerConfig, serverName: string): Promise<Client> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.CONNECT} Connecting to ${serverName} (${config.type})`,
      { serverName, transportType: config.type }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.CONNECT} Successfully connected to ${serverName}`,
        { serverName, timeout }
      );
  ...
  private async _createStdioTransport(config: StdioServerConfig): Promise<Transport> {
    ...
    this.logger.debug(
      `${LOG_PREFIXES.CONNECT} Creating stdio transport`,
      {
        command: resolvedCommand,
        args: resolvedArgs,
  ...
  private async _createSseTransport(config: SseServerConfig): Promise<Transport> {
    this.logger.debug(
      `${LOG_PREFIXES.CONNECT} Creating SSE transport`,
      { url: config.url, serverName: this.serverName }
    );
  ...
  private async _createHttpTransport(config: HttpServerConfig): Promise<Transport> {
    this.logger.debug(
      `${LOG_PREFIXES.CONNECT} Creating HTTP transport`,
      { url: config.url, serverName: this.serverName }
    );
  ...
  async callTool(name: string, args: any): Promise<ToolExecutionResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.TOOL} Calling tool: ${name}`,
      { toolName: name, serverName: this.serverName, timeout }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.TOOL} Tool executed successfully: ${name}`,
        { toolName: name, serverName: this.serverName }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.TOOL} ${ERROR_MESSAGES.TOOL_EXECUTION_FAILED}: ${name}`,
        { toolName: name, serverName: this.serverName, error: errorMessage }
      );
  ...
  async getTools(): Promise<ToolSet> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.TOOL} Failed to list tools`,
        { serverName: this.serverName, error: errorMessage }
      );
      throw error;
    }
  ...
  async listPrompts(): Promise<string[]> {
    ...
    try {
      ...
      this.logger.debug(
        `${LOG_PREFIXES.PROMPT} Retrieved ${promptNames.length} prompts`,
        { serverName: this.serverName, promptCount: promptNames.length }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.PROMPT} Failed to list prompts`,
        { serverName: this.serverName, error: errorMessage }
      );
      throw error;
    }
  ...
  async getPrompt(name: string, args?: any): Promise<GetPromptResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.PROMPT} Getting prompt: ${name}`,
      { promptName: name, serverName: this.serverName }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.PROMPT} Retrieved prompt successfully: ${name}`,
        { promptName: name, serverName: this.serverName }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.PROMPT} Failed to get prompt: ${name}`,
        { promptName: name, serverName: this.serverName, error: errorMessage }
      );
      throw error;
    }
  }
  ...
  async listResources(): Promise<string[]> {
    ...
    try {
      ...
      this.logger.debug(
        `${LOG_PREFIXES.RESOURCE} Retrieved ${resourceUris.length} resources`,
        { serverName: this.serverName, resourceCount: resourceUris.length }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.RESOURCE} Failed to list resources`,
        { serverName: this.serverName, error: errorMessage }
      );
      throw error;
    }
  ...
  async readResource(uri: string): Promise<ReadResourceResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.RESOURCE} Reading resource: ${uri}`,
      { resourceUri: uri, serverName: this.serverName }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.RESOURCE} Read resource successfully: ${uri}`,
        { resourceUri: uri, serverName: this.serverName }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.RESOURCE} Failed to read resource: ${uri}`,
        { resourceUri: uri, serverName: this.serverName, error: errorMessage }
      );
      throw error;
    }
  }
  ...
  private async _cleanup(): Promise<void> {
    ...
    if (this.client) {
      try {
        ...
        this.logger.warn(
          `${LOG_PREFIXES.CONNECT} Error during client cleanup`,
    ...
    if (this.transport) {
      try {
        ...
        // Log but don't throw cleanup errors
        this.logger.warn(
          `${LOG_PREFIXES.CONNECT} Error during transport cleanup`,
          { error: error instanceof Error ? error.message : String(error) }
        );
      }
      this.transport = null;
