...
export class MCPManager implements IMCPManager {
  ...
  registerClient(name: string, client: IMCPClient): void {
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Registered client: ${name}`,
  ...
  async executeTool(toolName: string, args: any): Promise<ToolExecutionResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Executing tool: ${toolName}`,
      { toolName, hasArgs: !!args }
    );
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.MANAGER} Tool execution failed: ${toolName}`,
  ...
  async listAllPrompts(): Promise<string[]> {
    ...
    const promptPromises = Array.from(this.clients.entries()).map(async ([name, entry]) => {
      ...
      try {
        ...
        this.logger.error(
          `${LOG_PREFIXES.MANAGER} Failed to list prompts from ${name}`,
          { clientName: name, error: errorMessage }
        );
      }
    });
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Retrieved ${promptList.length} total prompts`,
      { promptCount: promptList.length, clientCount: this.clients.size }
    );
  ...
  async getPrompt(name: string, args?: any): Promise<GetPromptResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Getting prompt: ${name}`,
      { promptName: name, hasArgs: !!args }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.MANAGER} Prompt retrieved successfully: ${name}`,
        { promptName: name }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.MANAGER} Failed to get prompt: ${name}`,
        { promptName: name, error: errorMessage }
      );
      throw error;
    }
  }
  ...
  async listAllResources(): Promise<string[]> {
    ...
    const resourcePromises = Array.from(this.clients.entries()).map(async ([name, entry]) => {
      ...
      try {
        ...
        this.logger.error(
          `${LOG_PREFIXES.MANAGER} Failed to list resources from ${name}`,
          { clientName: name, error: errorMessage }
        );
      }
    });
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Retrieved ${resourceList.length} total resources`,
      { resourceCount: resourceList.length, clientCount: this.clients.size }
    );
  ...
  async readResource(uri: string): Promise<ReadResourceResult> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Reading resource: ${uri}`,
      { resourceUri: uri }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.MANAGER} Resource read successfully: ${uri}`,
        { resourceUri: uri }
      );
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `${LOG_PREFIXES.MANAGER} Failed to read resource: ${uri}`,
        { resourceUri: uri, error: errorMessage }
      );
      throw error;
    }
  }
  ...
  async initializeFromConfig(serverConfigs: ServerConfigs): Promise<void> {
    ...
    const failedStrictServers = strictServers.filter(name => 
      this.failedConnections[name] !== undefined
    );

    if (failedStrictServers.length > 0) {
      const errorMessage = `${ERROR_MESSAGES.MISSING_REQUIRED_SERVERS}: ${failedStrictServers.join(', ')}`;
      this.logger.error(
        `${LOG_PREFIXES.MANAGER} ${errorMessage}`,
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Initialization complete`,
      { 
        successCount,
        failureCount,
  ...
  async connectServer(name: string, config: McpServerConfig): Promise<void> {
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Connecting to server: ${name}`,
      { serverName: name, transportType: config.type, connectionMode: config.connectionMode }
    );
    ...
    try {
      ...
      this.logger.info(
        `${LOG_PREFIXES.MANAGER} Successfully connected to server: ${name}`,
        { serverName: name }
      );
      ...
      this.logger.error(
        ...
        { serverName: name, error: errorMessage, connectionMode: config.connectionMode }
      );
  ...
  async removeClient(name: string): Promise<void> {
    ...
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Removing client: ${name}`,
      { clientName: name }
    );
    ...
    this.logger.info(
      ...
      { clientName: name, remainingClients: this.clients.size }
    );
  }
  ...
  async disconnectAll(): Promise<void> {
    this.logger.info(
      `${LOG_PREFIXES.MANAGER} Disconnecting all clients`,
