...
} from '../connection/transport-factory.js';
...
export class ServerRegistry {
		...
		return this.initializationLock.withLock(async () => {
			...
			try {
				...
				const finalSettings = settings ||
					...
					this.config.defaultSettings || { mcp: { servers: {} } };
				...
				this.logger.error(
					`Failed to initialize server registry: ${error instanceof Error ? error.message : String(error)}`
				);
				throw error;
			}
		});
	...
	private async loadServersFromSettings(settings: Settings): Promise<void> {
		...
		for (const [serverName, serverConfig] of Object.entries(settings.mcp.servers)) {
			try {
				...
				this.logger.error(
					`Failed to add server '${serverName}': ${error instanceof Error ? error.message : String(error)}`
				);
	...
	async *startServer(
		...
		const transportContext = await this.createTransportContext(
			entry.config,
			serverName,
			currentSessionId
		);
		...
		for await (const session of this.startServer(serverName, clientSessionFactory, sessionId)) {
			try {
				...
				if (hook) {
					...
					if (!shouldContinue) {
						this.logger.warning(
							`Init hook for server '${serverName}' returned false, skipping session`
						);
						return;
					}
				}
				...
				this.logger.error(
					`Failed to initialize session for server '${serverName}': ${error instanceof Error ? error.message : String(error)}`
				);
				throw error;
			}
		}
