...
export class ConnectionLifecycleManager {
    ...
    return this.connectionLock.withLock(async () => {
      ...
      if (this.connections.has(serverName)) {
        ...
        if (existingInfo.connection && existingInfo.state !== 'failed') {
          this.logger.debug(`Connection '${serverName}' already exists and is ${existingInfo.state}`);
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error during lifecycle manager shutdown: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
  ...
  getStatistics(): LifecycleStatistics {
    return {
      ...
      healthyConnections: Array.from(this.connections.values()).filter(
        info => info.connection?.isHealthy()
      ).length,
  ...
  private async runHealthMonitoring(
    ...
    try {
      ...
      this.logger.warning(`Health monitoring error for connection '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async attemptRecovery(serverName: string): Promise<boolean> {
    ...
    try {
      for (let attempt = 1; attempt <= this.config.maxRecoveryAttempts; attempt++) {
        this.logger.debug(`Recovery attempt ${attempt}/${this.config.maxRecoveryAttempts} for '${serverName}'`);
        ...
        try {
          ...
          if (info.connection.isHealthy()) {
            await this.updateConnectionState(serverName, 'running');
            await this.emitEvent('connection_recovered', serverName, { attempts: attempt });
          ...
          this.logger.warning(`Recovery attempt ${attempt} failed for '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      this.logger.error(`Recovery failed for '${serverName}' after ${this.config.maxRecoveryAttempts} attempts`);
      return false;
      ...
      this.logger.error(`Recovery error for '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  private async cleanupConnection(serverName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warning(`Error cleaning up connection '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
    } finally {
  ...
  private async emitEvent(
    ...
    for (const listener of listeners) {
      try {
        ...
        this.logger.warning(`Error in lifecycle event listener: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
