...
export class HealthMonitor {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.logger.warning(`Error waiting for monitoring tasks to complete: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  async triggerRecovery(): Promise<boolean> {
    return this.recoveryLock.withLock(async () => {
      ...
      await this.emitEvent('recovery_started', { attempts: this.config.maxRecoveryAttempts });

      try {
        for (let attempt = 1; attempt <= this.config.maxRecoveryAttempts; attempt++) {
          this.logger.debug(`Recovery attempt ${attempt}/${this.config.maxRecoveryAttempts} for '${this.targetName}'`);
          ...
          try {
            ...
            if (result.isHealthy) {
              this.logger.info(`Recovery successful for '${this.targetName}' after ${attempt} attempts`);
              await this.emitEvent('recovered', { attempts: attempt });
              return true;
            }

          } catch (error) {
            this.logger.warning(`Recovery attempt ${attempt} failed for '${this.targetName}': ${error instanceof Error ? error.message : String(error)}`);
          }
        }

        // All recovery attempts failed
        this.logger.error(`Recovery failed for '${this.targetName}' after ${this.config.maxRecoveryAttempts} attempts`);
        await this.emitEvent('recovery_failed', { attempts: this.config.maxRecoveryAttempts });
        return false;
  ...
  private async runMonitoringLoop(): Promise<void> {
    ...
    while (this.isRunning && !this.abortManager.aborted) {
      try {
        ...
        this.logger.error(`Error in monitoring loop for '${this.targetName}': ${error instanceof Error ? error.message : String(error)}`);
        
        // Wait a bit before retrying
        await new Promise(resolve => setTimeout(resolve, Math.min(this.config.checkInterval, 5000)));
      }
    }
  }
  ...
  private async emitEvent(type: HealthEventType, data?: any, error?: Error): Promise<void> {
    ...
    for (const listener of listeners) {
      try {
        ...
        this.logger.warning(`Error in health event listener for '${this.targetName}': ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
} 