...
export class ServerConnection {
	...
	async initialize(): Promise<void> {
		return this.connectionLock.withLock(async () => {
			...
			try {
				...
				this.logger.info(
					`Successfully initialized connection to server '${this.config.serverName}'`
				);
			} catch (error) {
		...
		this.healthCheckTaskId = this.taskGroup.startInBackground(async () => {
			...
			while (!this.shutdownRequested.isSet()) {
				try {
					...
					this.logger.warning(
						`Health check task error for server '${this.config.serverName}': ${error instanceof Error ? error.message : String(error)}`
					);
					break;
				}
			}
	...
	async performHealthCheck(): Promise<boolean> {
		...
		try {
			...
			await this.circuitBreaker.execute(async () => {
				...
				await Promise.race([this.client!.listPrompts(), timeoutPromise]);
			});
	...
	private async handleHealthCheckFailure(error: Error): Promise<void> {
		...
		this.logger.warning(
			`Health check failed for server '${this.config.serverName}' (${this.state.consecutiveFailures}/${this.healthCheckConfig.maxConsecutiveFailures}): ${error.message}`
		);

		// If we've exceeded max failures, trigger recovery
		if (this.state.consecutiveFailures >= this.healthCheckConfig.maxConsecutiveFailures) {
			this.logger.error(
				`Max consecutive health check failures reached for server '${this.config.serverName}', marking as unhealthy`
			);
	...
	private async handleConnectionError(error: Error): Promise<void> {
		...
		this.logger.error(
			`Connection error for server '${this.config.serverName}': ${normalizedError.getUserFriendlyMessage()}`
		);
	}
	...
	private async performShutdown(): Promise<void> {
		...
		try {
			...
			try {
				...
				this.logger.warning(
					`Error waiting for tasks to complete during shutdown: ${error instanceof Error ? error.message : String(error)}`
				);
			}
			...
			this.logger.error(
				`Error during shutdown for server '${this.config.serverName}': ${error instanceof Error ? error.message : String(error)}`
			);
		} finally {
	...
	private async cleanupSession(): Promise<void> {
		...
		if (this.client) {
			try {
				...
				this.logger.warning(
					`Error disconnecting client for server '${this.config.serverName}': ${error instanceof Error ? error.message : String(error)}`
				);
			}
			this.client = null;
		}
		...
		if (this.transport) {
			try {
				...
				this.logger.warning(
					`Error disposing transport for server '${this.config.serverName}': ${error instanceof Error ? error.message : String(error)}`
				);
			}
			this.transport = null;
		}
