...
export class TransportFactory {
	...
	static async createTransport(
		...
		this.logger.debug(
			`Creating transport for server '${creationConfig.serverName}' of type '${config.type}'`
		);
		...
		try {
			...
			switch (config.type) {
				case 'stdio':
					transport = await this.createStdioTransport(config as StdioServerConfig, creationConfig);
					break;

				case 'http':
					transport = this.createHttpTransport(config as HttpServerConfig, creationConfig);
					break;

				case 'sse':
					transport = this.createSSETransport(config as SseServerConfig, creationConfig);
					break;
			...
			this.logger.info(
				`Successfully created ${config.type} transport for server '${creationConfig.serverName}'`
			);
			return instance;
		} catch (error) {
			this.logger.error(
				`Failed to create transport for server '${creationConfig.serverName}': ${error instanceof Error ? error.message : String(error)}`
			);
	...
	private static validateConfig(config: McpServerConfig, serverName: string): void {
		if (!config) {
			throw new ConfigurationError('Server configuration is required', serverName, 'config');
		}

		if (!config.type) {
			throw new ConfigurationError('Server type is required', serverName, 'type');
		}
	...
	private static validateHttpConfig(config: HttpServerConfig, serverName: string): void {
		if (!config.url) {
			throw new ConfigurationError('URL is required for HTTP transport', serverName, 'url');
		}
	...
	private static validateSseConfig(config: SseServerConfig, serverName: string): void {
		if (!config.url) {
			throw new ConfigurationError('URL is required for SSE transport', serverName, 'url');
		}
	...
	static async createTransportWithRetry(
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				this.logger.warning(
					`Transport creation attempt ${attempt}/${maxAttempts} failed for server '${creationConfig.serverName}': ${lastError.message}`
				);
	...
	static async disposeTransport(instance: TransportInstance): Promise<void> {
		try {
			if (instance.transport && typeof instance.transport.close === 'function') {
				...
				this.logger.debug(
					`Disposed ${instance.type} transport for server '${instance.metadata.serverName}'`
				);
			}
		} catch (error) {
			this.logger.warning(
				`Error disposing transport for server '${instance.metadata.serverName}': ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
}
