...
import { ConnectionFailureError } from '../errors/connection-errors.js';
import {
...
export class MCPConnectionManager {
	...
	async initialize(serverConfigs: ServerConfigs, context?: IContext): Promise<void> {
		return this.initializationLock.withLock(async () => {
			...
			try {
				...
				for (const [serverName, serverConfig] of Object.entries(serverConfigs)) {
					connectionPromises.push(
						this.startServerConnection(serverName, serverConfig)
							...
							.catch(error => {
								this.logger.warning(
									`Failed to start connection for server '${serverName}': ${error instanceof Error ? error.message : String(error)}`
								);
				...
				this.logger.error(
					`Failed to initialize connection manager: ${error instanceof Error ? error.message : String(error)}`
				);
				throw error;
			}
		});
	...
	async getServerConnection(
		...
		try {
			...
			if (options.requireHealthy && !connection.isHealthy()) {
				throw new ConnectionFailureError(`Server '${serverName}' is not healthy`, serverName);
			}
	...
	async getStatistics(): Promise<ConnectionPoolStats> {
		...
		return {
			...
			idleConnections: Math.max(
				0,
				lifecycleStats.activeConnections - this.getCurrentlyUsedConnections()
			),
		...
		for (const [serverName, info] of connections) {
			if (info.connection) {
				try {
					...
					this.logger.error(
						`Health check failed for server '${serverName}': ${error instanceof Error ? error.message : String(error)}`
					);
	...
	async warmupConnections(): Promise<void> {
		...
		for (const [serverName, info] of connections) {
			if (info.connection) {
				warmupPromises.push(
					this.warmupConnection(serverName, info.connection).catch(error => {
						this.logger.warning(
							`Failed to warm up connection for server '${serverName}': ${error instanceof Error ? error.message : String(error)}`
						);
					})
				);
			}
	...
	async shutdown(): Promise<void> {
		...
		try {
			...
			this.logger.error(
				`Error during connection manager shutdown: ${error instanceof Error ? error.message : String(error)}`
			);
		} finally {
	...
	private async warmupConnection(serverName: string, connection: ServerConnection): Promise<void> {
		try {
			...
			await Promise.allSettled([client.getTools(), client.listPrompts(), client.listResources()]);
