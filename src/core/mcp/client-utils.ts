  ...
  try {
    ...
    if (options?.timeout) {
      await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error(`Connection timeout after ${options.timeout}ms`)), options.timeout)
        ),
      ]);
    } else {
      await client.connect(serverConfig, serverName);
    ...
    if (client.disconnect) {
      try {
        ...
        logger.warning(`Error disconnecting temporary client: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
}
