...
describe('MCPManager', () => {
  ...
  describe('Tool Management', () => {
    ...
    it('should throw error when executing non-existent tool', async () => {
      await expect(manager.executeTool('non-existent-tool', {}))
        .rejects
        .toThrow(ERROR_MESSAGES.NO_CLIENT_FOR_TOOL);
    });
    ...
    it('should handle tool execution errors', async () => {
      ...
      await expect(manager.executeTool('failing-tool', {}))
        .rejects
  ...
  describe('Prompt Management', () => {
    ...
    it('should get a prompt successfully', async () => {
      ...
      mockClient.getPrompt.mockResolvedValue({ messages: [{ role: 'user', content: { type: 'text', text: 'test' } }] });
    ...
    it('should throw error when getting non-existent prompt', async () => {
      await expect(manager.getPrompt('non-existent-prompt'))
        .rejects
        .toThrow(ERROR_MESSAGES.NO_CLIENT_FOR_PROMPT);
    });
  });
  ...
  describe('Resource Management', () => {
    ...
    it('should throw error when reading non-existent resource', async () => {
      await expect(manager.readResource('file://non-existent.txt'))
        .rejects
        .toThrow(ERROR_MESSAGES.NO_CLIENT_FOR_RESOURCE);
    });
  });
  ...
  describe('Connection Management', () => {
    it('should initialize from config with mixed connection modes', async () => {
      ...
      await expect(manager.initializeFromConfig(serverConfigs))
        .resolves
        .not
        .toThrow();
    ...
    it('should fail when strict server cannot connect', async () => {
      ...
      await expect(manager.initializeFromConfig(serverConfigs))
        .rejects
        .toThrow(ERROR_MESSAGES.MISSING_REQUIRED_SERVERS);
    });
    ...
    it('should handle connection failures for lenient servers', async () => {
      ...
      await expect(manager.connectServer('failing-server', config))
        .resolves
        .not
        .toThrow();
    ...
    it('should handle removal of non-existent client gracefully', async () => {
      await expect(manager.removeClient('non-existent-client'))
        .resolves
        .not
        .toThrow();
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle client failures gracefully when getting tools', async () => {
      ...
      mockClient.getTools.mockResolvedValue({ 'tool1': { description: 'Tool 1', parameters: { type: 'object', properties: {} } } });
    ...
    it('should throw error when all clients fail to get tools', async () => {
      ...
      await expect(manager.getAllTools())
        .rejects
        .toThrow('Failed to retrieve tools from all clients');
    });
