...
export class ResourceMaps {
  ...
  async addTool(
    ...
    await this.toolMapLock.withLock(async () => {
      ...
      const namespacedName = createNamespacedName(serverName, toolName, opts.namespacing?.separator);
  ...
  async addPrompt(
    ...
    await this.promptMapLock.withLock(async () => {
      const opts = { ...this.options, ...options };
      const namespacedName = createNamespacedName(serverName, promptName, opts.namespacing?.separator);
  ...
  async addPrompts(
    ...
    const addPromises = prompts.map(prompt => {
      if (typeof prompt === 'string') {
        ...
        return this.addPrompt(serverName, prompt.name, {
  ...
  async addResource(
    serverName: string,
    resourceUri: string,
    resourceInfo: { name?: string; description?: string; mimeType?: string } = {},
    options?: ResourceMapOptions
  ): Promise<void> {
    await this.resourceMapLock.withLock(async () => {
      const opts = { ...this.options, ...options };
      const namespacedUri = createNamespacedName(serverName, resourceUri, opts.namespacing?.separator);
  ...
  async addResources(
    serverName: string,
    resources: string[] | Array<{ uri: string; name?: string; description?: string; mimeType?: string }>,
    options?: ResourceMapOptions
  ): Promise<void> {
    const addPromises = resources.map(resource => {
      if (typeof resource === 'string') {
        ...
        return this.addResource(serverName, resource.uri, {
          name: resource.name,
