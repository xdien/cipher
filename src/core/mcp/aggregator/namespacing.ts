 ...
 */
export const RESERVED_PREFIXES = new Set(['system', 'internal', 'aggregator', 'mcp', 'protocol']);
...
export function extractServerName(
	...
	const parsed = parseNamespacedName(namespacedName, serverNames, {
		separator,
	});
	return parsed.serverName;
}
...
export function validateServerName(serverName: string): void {
	...
	if (!/^[a-zA-Z][a-zA-Z0-9-]*$/.test(serverName)) {
		throw new Error(
			'Server name must start with a letter and contain only letters, numbers, and hyphens'
		);
	}
