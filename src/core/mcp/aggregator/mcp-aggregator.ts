...
export class MCPAggregator {
  ...
  async initialize(
    ...
    return this.initializationLock.withLock(async () => {
      ...
      try {
        ...
        this.logger.error(`Failed to initialize MCP aggregator: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
      }
    });
  }
    ...
    if (this.config.enableParallelLoading) {
      ...
      const loadPromises = this.serverNames.map(serverName =>
        this.loadServer(serverName).catch(error => {
          this.logger.error(`Failed to load server '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
  ...
  async loadServer(serverName: string): Promise<void> {
    ...
    try {
      ...
      if (resourcesResult.status === 'rejected') {
        this.logger.warning(`Failed to load resources from '${serverName}': ${resourcesResult.reason}`);
      }

      this.logger.debug(`Successfully loaded capabilities from server: ${serverName}`);

    } catch (error) {
      this.logger.error(`Error loading server '${serverName}': ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error during aggregator shutdown: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async updateStatistics(): Promise<void> {
    try {
      ...
      this.logger.debug(`Error updating statistics: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
