...
export class MCPAgentClientSession extends MCPClient implements IEnhancedMCPClient {
	...
	async sendProgressNotification(token: string, progress: number, total?: number): Promise<void> {
		this.sessionLogger.debug(
			`Sending progress notification: ${progress}${total ? `/${total}` : ''} for token ${token}`
		);
	...
	private async handleSamplingCallback(params: SamplingParams): Promise<SamplingResult> {
		...
		try {
			...
			if (this.upstreamSession) {
				...
				if (typeof this.upstreamSession.handleSampling === 'function') {
					try {
						...
						this.sessionLogger.warning(
							`Upstream session can't handle sampling: ${error instanceof Error ? error.message : String(error)}`
						);
					}
				}
				...
				try {
					...
					this.sessionLogger.warning(
						`Error using upstream session for sampling: ${error instanceof Error ? error.message : String(error)}`
					);
				}
			}
			...
			this.sessionLogger.warning(
				'No upstream session available for sampling, using mock implementation'
			);
			...
			this.sessionLogger.error(
				`Error in sampling callback: ${error instanceof Error ? error.message : String(error)}`
			);
			return {
				text: '',
	...
	private async handleListRootsCallback(): Promise<Root[]> {
		...
		try {
			...
			if (config && config.roots && Array.isArray(config.roots)) {
				this.sessionLogger.debug(
					`Returning ${config.roots.length} roots from context configuration`
				);
				return config.roots;
			}
			...
			this.sessionLogger.error(
				`Error in list_roots callback: ${error instanceof Error ? error.message : String(error)}`
			);
			return [];
		}
	}
